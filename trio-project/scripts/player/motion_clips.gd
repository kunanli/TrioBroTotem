...
const IDLE := {
	...
	# 法師：站得最直，**法杖斜立在身前、雙手都握在杖身上**。
	#
	# 右上臂的 Z 是**大的正值**，跟另外兩隻相反：這隻的名冊 pose 已經把右臂
	# 往內收了 52 度（那是為了修骨架本身手張太開），照抄另外兩隻的寫法會把
	# 手臂插進軀幹。+74 是把它扳回來，淨值 −52 度。
	#
	# **手臂的 X 是為了讓左手搆得到法杖才調到 −58／−46 的，不是為了好看。**
	# 原本法杖立在身側（右上臂 X −24），左肩到杖身握點是 1.07 倍臂長——
	# 差那 7% 就是「手在法杖旁邊比劃」而不是「握著法杖」。把右臂往前送、
	# 法杖朝向改成偏前偏左（`weapon_aim.TARGETS`）之後降到 0.94 倍，
	# `hand_ik.gd` 才收得掉最後那幾公分（實測殘差 1.3 公分）。
	#
