...
##
## **第一版是 0.8，壓過頭了**：劍在走路的每一格裡一模一樣，像焊在鏡頭上的道具。
## 用 `hand_probe` 掃出來的（尖端行程占「不穩定」時的幾成，走／跑／衝刺）：
##
##     0.8 / 0.35 s    30–40%   看起來是死的
##     0.6 / 0.35 s    42–53%   ← 這個：剛好壓掉一半，圖上看得出劍跟著步子輕擺
##     0.6 / 0.25 s    47–60%
##     0.5 / 0.35 s    52–66%
##
## 而且 `gait_bob` 的身體起伏排在這一層前面，會被這裡的平滑吃掉一部分——那正好
