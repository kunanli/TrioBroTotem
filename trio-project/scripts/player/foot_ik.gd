...
##
## 解算器在 `limb_ik.gd`（跟 `hand_ik.gd` 共用）。為什麼是自己寫的 CCD 而不是
## 內建的 `TwoBoneIK3D`，寫在那個檔案的檔頭。
##
## ## 讀寫的都是「修改器堆疊裡面」的姿勢
##
...
##
## **例外是 `BoneAttachment3D`**：它讀的是堆疊跑完的結果，所以掛在手上的武器
## 會跟著 `hand_ik` 走（實測驗過）。腳上沒有掛東西，這一層用不到這件事。
...
func _build(skeleton: Skeleton3D) -> void:
	...
	for entry in LEGS:
		...
		var chain := LimbIk.find_chain(skeleton, [names["upper"], names["lower"]])
		var foot := skeleton.find_bone(String(names["foot"]))
		if chain.is_empty() or foot < 0:
			continue
		_legs.append({
			"chain": chain,
			"foot": foot,
			"length": LimbIk.chain_length(skeleton, chain, foot),
			"planted": false,
			"locked": Vector3.ZERO,
			"ankle": Vector3.ZERO,
			"world": Vector3.ZERO,
