...
## 身體有多粗（`BodyShape.measure()`）：防撞層與探針都讀這一份。
var _body_shape: Dictionary = {}
...
func load_character(id: StringName) -> bool:
	...
	_attach_pose(entry)
	CullBounds.fix(_skeleton, String(character_id), float(entry.get("height", 1.8)))
...
## 防撞層。**堆疊的最後一層**：它要看到的是所有姿勢與 IK 疊完之後兩隻手在哪，
## 然後才知道撞沒撞。身體形狀從網格量一次（`BodyShape`），量不到就用身高比例
## 的預設並警告——不能靜靜地用 0，那等於沒有這一層。
func _attach_arm_guard() -> void:
	if _skeleton == null:
		return
	_body_shape = BodyShape.measure(_skeleton)
	if not bool(_body_shape["ok"]):
		push_warning("[Visual] %s 量不到身體形狀，防撞用身高比例的預設" % character_id)
	_arm_guard = ArmGuard.new()
	_arm_guard.name = "ArmGuard"
	_arm_guard.configure(_body_shape)
	_skeleton.add_child(_arm_guard)


## 身體形狀（`BodyShape.measure()` 的結果）。探針印出來當佐證。
...
func _drive_hand_ik() -> void:
	...
	# 防撞在布娃娃期間關掉（那時候是物理在擺手），其餘全開——攻擊、扛東西、
	# 滯空正是手最會打架的時候。
	if _arm_guard != null:
		_arm_guard.set_enabled(_ragdoll == null or not _ragdoll.active)
		_arm_guard.set_gripping(
			_hand_ik.grip_distance() if _hand_ik != null and _hand_ik.is_steadying() else 0.0
		)
