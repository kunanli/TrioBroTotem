...
func _build_bank() -> void:
	...
	# 跳躍：短促上揚，跟落地的下掃剛好相反，一聽就知道是上去還是下來。
	_bank[&"jump"] = _sweep(210.0, 380.0, 0.09, 0.004)
	# 腳步：極短的悶噪音。一秒會響好幾次，所以要短、要悶、要能被壓小聲。
	_bank[&"step"] = _noise(0.055, 0.002, 0.78)
	# 弦響：高頻快速下滑再加一點噪音，就是弦彈回去的那一下。弓手放箭的聲音，
	# 代替揮空音——弓不會「揮」。
	_bank[&"twang"] = _mix(_sweep(1400.0, 500.0, 0.08, 0.002), _noise(0.05, 0.002, 0.5), 0.35)
	# 咒音：低頻往上爬，有點厚。法杖出手那一刻放，跟球的閃光同一幀。
	_bank[&"cast"] = _mix(_sweep(110.0, 330.0, 0.22, 0.02), _noise(0.12, 0.01, 0.3), 0.3)
	# 門沉下去：很長的低頻下掃摻石頭摩擦的噪音。
	# **門常常不在畫面裡**——壓力板在這一頭、門在二十公尺外，所以這一聲是
	# 「你剛剛做的事有用」唯一的回饋，要長到讓人來得及轉頭。
	_bank[&"gate"] = _mix(
		_sweep(120.0, 46.0, 1.10, 0.05), _noise(1.00, 0.06, 0.22), 0.55
	)
