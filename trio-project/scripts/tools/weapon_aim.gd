...
##
## **法杖那一個後來被改成偏前偏左，而且是量出來的。** 原本是 (0.42, 0.90, −0.12)
## ——立在身側偏前，好看，但左肩到杖身握點是 1.07 倍臂長，左手永遠搆不到。
## 改成往前往左之後降到 0.94 倍，`hand_ik.gd` 才接得上。這個檔案原本只管
## 「武器看得見」，現在還多管一件「副手構得到」——改任何一個目標方向之後，
## 除了重跑這支，還要跑 `hand_probe` 看那個比例。
const TARGETS := {
	&"pig_warrior": Vector3(0.70, 0.68, 0.22),   # 劍扛在肩後，往上偏右偏後
	&"frog_mage": Vector3(0.45, 0.86, -0.24),   # 法杖立在身前偏右，雙手都握得到
