...
const TARGETS := {
	...
	# 法杖橫抱在胸前偏左、往前伸，雙手分開握。以前是 (-0.85, 0.25, -0.46)，握點在
	# 肚子裡 18 公分（`hand_probe` 的 [Clash]）——副手接上了，但接在身體裡面。
	# 再往前（-0.32, 0.22, -0.92）待機就全在身體外，但走路時左肩到握點 1.04 倍臂長，
	# 副手放開；這一組是折衷：待機差 1.3 公分（防撞補掉）、走路 0.9x 倍搆得到。
	&"frog_mage": Vector3(-0.42, 0.22, -0.88),
