...
scenes/
  ...
  tools/hand_probe.tscn  手部量尺：武器甩多少、副手離武器多遠、兩手打不打架（--quick 只量待機）
scripts/
  autoload/
...
改完跑 `godot --headless --fixed-fps 120 --path trio-project res://scenes/tools/beat_probe.tscn`，
...
### 左右手打架（`arm_guard.gd`、`body_shape.gd`）

「左右手經常打架」——走跑衝、攻擊、待機轉身都有，三隻都有。先排除走路片段本身：
從 GLB 做正向運動學，兩手最近 64–94 公分、手離軀幹軸 31–53 公分，把跑步／衝刺的
手臂放大也還有 63 公分——**Meshy 的走路不打架**。打架來自後面疊上去的每一層：職業
姿態、副手 IK 把手拉去握點、生成的攻擊姿勢、鏡像的第二段、轉身傾斜、扛東西。每一支
都是人寫的角度，沒有任何一層知道另一隻手在哪、軀幹有多粗。

所以不逐格改角度（改了一格下一支動作又撞），而是在修改器堆疊的**最後**放一層知道
身體形狀的守門員：`ProceduralPose → Ragdoll → RagdollRecovery → GaitBob → FootIk →
HandIk → ArmGuard`。手撞到軀幹、頭、另一隻手就推開，逐幀保證不穿。

**量尺先做**：`hand_probe` 多一張 `[Clash]` 表——每個速度帶、每一支動作，印兩手重疊
幾公分、手穿軀幹／穿頭幾公分（穿多深＝要推多遠才出來）、肘穿軀幹幾公分（只印不推）。
`--guard=0` 關掉防撞當對照組。修前的地圖（各段最深的，公分）：

| | 兩手重疊 | 手穿軀幹（左／右） | 最糟的一段 |
|---|---|---|---|
| 豬 | 0 | 17 ／ 27 | 三段攻擊、衝刺撞擊：劍手整隻插進肚子 |
| 蛙 | 12 | 23 ／ 16 | 每一段攻擊；**待機副手在肚子裡 18 公分** |
| 貓 | 9 | 0 ／ 13 | 走跑衝兩手黏在一起，跳躍右手穿身 |

修後三隻**每一段**的兩手重疊、手穿軀幹、手穿頭都是 0（`hand_probe` 的 `[Clash]`
沒有一行標「打架」）；肘穿軀幹沒推，青蛙出招時左肘還在身體裡 15 公分，表上看得到。
尖端行程、`[Strike]` 的髖與回招時間跟修前一樣——防撞在不打架的姿勢不出手。

**身體多粗是從網格量的**（`body_shape.gd`），不寫死在名冊：三隻的胸寬、頭大小差
很多，而且看骨頭猜不出來——大頭身的肩骨貼在脊椎旁，兩肩距離只有 3–4 公分。讀蒙皮
頂點與權重，每個頂點歸給權重最大的骨，量三種東西：軀幹是**繞軸 16 個方向、上下
兩段的剖面**，頭一顆球，手一顆球。第一版四個數字全量錯，每一個都是一課：

- 軀幹用「主骨是 Hips 的頂點到線段的距離」量成 59–61 公分——Meshy 把腿的一半權重
  給了 Hips（腳離髖 40 公分），肩高的手臂上也有 Hips 為主的頂點。改成用**位置**挑
  （投影在軸的 5%–85% 之間、又不是四肢或頭），不信權重。
- 單一半徑再被豬的尾巴撐到 61：正後方那一格 61、兩鄰 31。改成分方向、再做環狀三格
  中位數，只佔一格的附屬物就消失；青蛙背後的袍子佔三格，留著。
- 頭用「到頭心的 95 百分位」被貓的耳朵撐到 67 公分——改用包圍盒的平均半長。
- 手用「到手骨原點的距離」把整截袖子算進來（青蛙的手 30 公分）——改量到前臂軸線的
  距離、取中位數，是手的厚度（三隻都 7.5 公分左右）。

量出來：豬軀幹 22–32、蛙 16–28、貓 10–17，頭 41／42／49，手 7.6／7.5／7.7 公分。
`beat_probe` 驗這幾個數字在合理範圍，骨名對不上會叫。

**防撞怎麼推**（`arm_guard.gd`）。每幀對每隻手：算它穿進軀幹剖面、頭球、另一隻手球
多深；然後找一個「在身體外面、又搆得到」的點。這一步改了三版：

1. 沿最短方向推——大頭身的手臂短，出招時本來就伸直了，推出去就超過臂長，CCD 把
   目標剪回去、手還在肚子裡（殘差 1–7 公分）。
2. 「推出身體、拉回球面」交替投影——手指著身體正下方時來回打轉（青蛙過頂重砍仍穿
   23 公分）；身體的外面不是凸集。
3. **先直接推、推不到就把手臂繞著肩膀轉開**：穿 1 公分推 1 公分，推完還在臂長內就用
   它（待機時副手只差 2.8 公分，這一步就夠）；超出臂長才掃：在四個平面（推出方向、
   身體前後、左右、上下各張一個）兩個轉向裡 5 度一步往外掃，第一個不穿的角度二分
   磨細，挑轉得最少的；先試現在的伸展長度、再試臂長上限。只掃一個平面的話，推的
   方向剛好沿著手臂時那個平面是隨便選的——青蛙待機的副手曾被轉到 56 公分外才找到
   出口。這才三隻全過。

推力**沒有阻尼**，每幀算多少推多少。試過兩種阻尼都錯：進去時阻尼（0.04 秒）讓攻擊
蓄力格一幀插進身體 25–40 公分、推力追了十幾幀才到；放開時阻尼更糟——留著上一幀的
推力向量，姿勢一轉那個方向就指進身體，量到的是**防撞層自己把手推進肚子 10 公分**
（青蛙攻擊 30 公分的穿身就是它）。穿多深→推多遠本來就是姿勢的連續函數，不需要阻尼。
CCD 前先用餘弦定理把手肘彎到剛好的角度——打直的手臂 CCD 沒轍（肘→手跟肘→目標
同方向，旋轉趨近零），貓的手臂伸到 0.41 倍、上限 0.40，六輪還差 5 公分，預彎之後
歸零。手骨的朝向寫回解之前的值：武器焊在手上。

**持械手是錨**，手對手只有空手讓。副手握在雙手武器上時（`HandIk` 在穩定），兩手
允許靠到握點那麼近——不然就是跟 IK 打架；握點遠的（法杖 38 公分）照推，IK 淡入時
副手橫過身體那幾幀才不會撞上持械手。**弓的握點順手往後移了 7.6 公分**
（`WeaponRack.OFF_GRIPS`）：以前拉弦的手離持弓的手骨只有 9 公分，兩隻 7.7 公分的
大手掌走跑時疊成一團（`[Clash]` 兩手 6.7）；現在弦在待機時微微搭著手、兩手剛好
分開，箭的顯示門檻跟著抬（`bow_string.gd`）。

**青蛙的待機順手修了一格**：副手接上了法杖，但握點在肚子裡 18 公分——之前只量
「接沒接上」沒量「在不在身體外」。防撞開著的話副手被推出來、離握點 24 公分，比手
插在肚子裡好但不對。把法杖往前伸（`weapon_aim.gd` 的 TARGETS 從 (−0.85, 0.25, −0.46)
改成 (−0.42, 0.22, −0.88)，名冊 spin 跟著重解）之後握點只差 1.3 公分（防撞補掉）、
副手接上、法杖離頭從 0.20 個身高變 0.26。再往前伸走路時左肩到握點會超過臂長
（1.04 倍，副手放開），所以停在這裡；走路時仍差 3%，`HandIk.RELEASE` 從 1.15 拉到
1.25，副手離杖身最遠從 15.6 公分降到 9.1。

不推的：前臂對軀幹（表上有印）、武器本身對身體（劍穿身是另一題）、腿。弓弦讀的
`HandIk.draw_point` 在防撞之前發佈，拉弓那隻手離身體遠，防撞幾乎不動它。

### 臉會被手拉歪（`tools/fix_skin_bleed.py`）

「青蛙的臉過度扭曲」不是頭骨的問題——把頭穩定層整個關掉（`GaitBob.HEAD_STEADY` 0）
畫面一模一樣。拆開 GLB 才看到：**Meshy 的自動蒙皮把手臂的權重滲進了整顆頭**。
大頭身的頭直接壓在肩膀上，權重是在體積裡擴散的，肩與上臂就這樣滲到臉、甚至頭頂
（1.4 公尺高的地方也有）。手一擺臉就剪切；待機時手不動看不出來，跑步的手臂擺幅
放大之後就整張臉歪掉。

| | 頂點 | 頭為主、卻帶手臂權重的 | 那份手臂權重（平均／最高） |
|---|---|---|---|
| 蛙 | 10,726 | 2,294 | **0.24 ／ 0.68** |
| 豬 | 11,109 | 2,505 | 0.16 ／ 0.58 |
| 貓 | 11,164 | 4,128 | 0.08 ／ 0.56 |

青蛙的臉每個頂點平均有四分之一是跟著手臂走的，難怪。反方向也漏：青蛙後腦與兩側
各有一片「上臂 0.5／頭 0.5」跟「頭 0.5／上臂 0.5」交錯散佈的頂點。

**第一版修法失敗，值得記著**：「主骨兩步外的骨骼權重一律搬走」在那片交錯區域裡讓
相鄰兩個頂點一個 100% 跟頭、一個 100% 跟手臂，後腦勺整片黑色撕裂。這種事要量：
相鄰頂點權重向量的 L1 差，99 百分位原檔 1.13，硬規則 **2.00**。任何逐頂點的硬分類
都會撕，修法必須是連續的。

現在的作法是**頭殼場**：每個頂點算「頭殼程度」f∈[0,1]。錨點只看兩件事：完全沒有
頭權重的頂點不是頭；帶頭權重、又離所有外來骨段超過身高 0.17 的頂點是頭（青蛙兩側
那片 0.5／0.5 離肩膀很遠，用權重猜不出來、用距離一看就是頭）。其餘那一圈沿網格
鄰接做諧和插值、再抹六輪。然後用**上限**：外來權重 ≤ 1−f（超過的搬到 Head）、
外來為主的頂點頭權重 ≤ f。頭殼上外來權重歸零、肩膀不動、脖子根漸變。錨點刻意
不看權重大小、規則不會把頭權重加到原本沒有的頂點上，所以**重跑不再變**——第一版
用權重當錨，修完場跟著長，一路搬到撕裂為止。

修完三隻頭殼上帶外來權重的頂點都是 0，撕裂 p99 蛙 1.15、豬 0.66、貓 0.65（原檔
1.13／0.65／0.65）。`check_project.py` 從此守著：新的 Meshy 匯出檔進來一定會再中，
它會叫你重跑 `--apply` 再 `--import`。

```bash
python3 tools/fix_skin_bleed.py          # 只量：症狀數、撕裂量尺
python3 tools/fix_skin_bleed.py --apply  # 改 GLB，然後 godot --headless --import
```

骨架沒動、動畫沒動，所以 `gait_probe`／`hand_probe`／`beat_probe` 的數字一個都不變
——這是純粹「網格跟不跟得對骨頭」的問題，量尺量的是骨頭。
...
- **蒙皮權重**：角色 GLB 的頭殼上有沒有手臂等外來骨骼的權重（見「臉會被手拉歪」）
- **身體形狀**（在 `beat_probe`）：三隻量出來的軀幹／頭／手半徑在合理範圍
