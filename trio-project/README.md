...
- **鏡頭方向固定，沒有滑鼠視角，也沒有 SpringArm。** 鏡頭規則尚未定案
  （見〈連線、營地與分屏〉鏡頭規則），定案前不做鏡頭相對移動，免得改兩次。
  M0 的測試場地是開闊地形，`SpringArm3D` 的價值（避免鏡頭穿牆）用不到，
  卻會引進投射方向的正負號慣例——第一次跑就因此把鏡頭放到角色前面。
  改成固定偏移：角色後上方 7 公尺、高 3.5 公尺、俯角 20 度。
